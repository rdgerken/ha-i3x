...
- **Service-backed content**: domains whose real content hides behind a service call expose it in their value object: `todo` lists carry their actual items, `calendar` entities the next seven days of events, and `weather` entities their forecast (daily preferred, else hourly). Fetched live with a freshness cache keyed on the entity's last update plus a short TTL.
